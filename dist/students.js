/* =========================================================
👥 STUDENTS PAGE MODULE — FINAL (LIFECYCLE + NO DUPLICATES)
- ✅ Sayfa açılınca: init (event bind + render)
- ✅ Sayfadan çıkınca: destroy (event unbind + cleanup)
- ✅ Duplicate event / memory leak yok
- ✅ Hem basit #studentsTable hem de gelişmiş #dataTable yapısını destekler
========================================================= */
...
(function () {
  ...
  // =========================================================
  // 0) DEMO SIMPLE TABLE (opsiyonel/fallback)
  // =========================================================
  function renderSimpleDemoIfPresent() {
  ...
  let ogrenciler = [
    {
      ...
      ad_soyad: "Ad-Soyad",
      adres:
        "AdresAdresAdresAdres AdresAdresAdresAdres AdresAdresAdresAdresAdres AdresAdres",
      sinif: "1E",
      sabah: "07:25",
      aksam: "16:25",
  ...
  // =========================================================
  // 2) LIFECYCLE HOOKS (App.js buradan çağırır)
  // =========================================================
  window.StudentsPageInit = function () {
    ...
    // Eğer sadece basit tablo varsa (studentsTable), onu doldur (fallback)
    // Ama gelişmiş tablo varsa (#dataTable) onu çalıştır.
    const hasAdvancedTable =
      document.getElementById("dataTable") ||
      document.getElementById("tableBody") ||
      document.querySelector(".tbl-header") ||
      document.querySelector(".tbl-content");

    if (!hasAdvancedTable) {
      renderSimpleDemoIfPresent();
      // yine de event bağlamayalım; çünkü advanced DOM yok.
      return;
    }
  ...
  // =========================================================
  // 3) EVENT BIND / UNBIND (NAMESPACE’li)
  // =========================================================
  function bindEvents() {
    // 2) TABLO — SIRALAMA (FINAL)
    $(".tbl-header")
      .off("click.students", "th[data-col]")
      .on("click.students", "th[data-col]", function () {
    ...
    $(".tbl-content")
      .off("scroll.students")
      .on("scroll.students", function () {
        $(".tbl-header").scrollLeft($(this).scrollLeft());
      });

    // 5) CHILD ROW — DETAY
    $(document)
      .off("click.students", "#dataTable tbody tr")
      .on("click.students", "#dataTable tbody tr", function (e) {
    ...
    // buton da açsın
    $(document)
      .off("click.students", ".more-btn")
      .on("click.students", ".more-btn", function (e) {
        e.stopPropagation();
    ...
    $(".pagination")
      ...
      .on(
        "click.students",
        ".page-item.number:not(.active):not(.disabled)",
        function () {
          showPage(parseInt($(this).data("page")));
        }
      )
      .on("click.students", ".page-item.first:not(.disabled)", () =>
        showPage(1)
      )
      .on("click.students", ".page-item.last:not(.disabled)", () => {
        const maxRows = parseInt($("#maxRows").val());
        showPage(Math.ceil(filtered.length / maxRows));
      })
      .on("click.students", ".page-item.prev:not(.disabled)", () =>
        showPage(currentPage - 1)
      )
      .on("click.students", ".page-item.next:not(.disabled)", () =>
        showPage(currentPage + 1)
      );
    ...
    // #maxRows change renderTable (senin kodun)
    $("#maxRows").off("change.studentsRender").on("change.studentsRender", function () {
      renderTable();
    });
    ...
    // 12) DÜZENLE
    $(document)
      .off("click.students", ".edit")
      .on("click.students", ".edit", function (e) {
        e.stopPropagation();
    ...
    // 13) SİL
    $(document)
      .off("click.students", ".delete")
      .on("click.students", ".delete", function (e) {
        e.stopPropagation();
    ...
    // 15) EXPORT / PDF / COPY / PRINT
    $("#exportExcel")
      .off("click.students")
      .on("click.students", function () {
        exportToExcel();
      });
    ...
    $(".modal-close, .student-modal-overlay")
      .off("click.students")
      .on("click.students", () => {
  ...
  function unbindEvents() {
    $(".tbl-header").off("click.students");
    $(".tbl-content").off("scroll.students");
    $(document).off("click.students", "#dataTable tbody tr");
    $(document).off("click.students", ".more-btn");

    $(".pagination").off("click.students");

    $("#maxRows").off("change.students");
    $("#maxRows").off("change.studentsRender");
    $("#searchInput").off("keyup.students");

    $("#addNewBtn").off("click.students");
    $("#btnSave").off("click.students");
    $(document).off("click.students", ".edit");
    $(document).off("click.students", ".delete");
    $("#confirmDeleteBtn").off("click.students");
    $("#refreshBtn").off("click.students");

    $("#exportExcel").off("click.students");
    $("#exportPdf").off("click.students");
    $("#copyTable").off("click.students");
    $("#printTable").off("click.students");
    $("#confirmPrint").off("click.students");

    $("#excelImportBtn").off("click.students");
    $("#excelFileInput").off("change.students");

    $("#addStudentBtn").off("click.students");
    $(".modal-close, .student-modal-overlay").off("click.students");
    $("#saveStudentBtn").off("click.students");
...
<td colspan="${colspan}">
    ...
    <div class="detail-content">
      ...
      <div class="detail-grid">
        ...
        <div class="detail-srv-center">
          <button type="button" class="split-btn info student-btn">
        ...
        <div class="detail-route-center">
          <button class="split-btn secondary route-detail-btn">
            <span class="icon-part"><i class="fa-solid fa-route"></i></span>
            <span class="label-part">Güzergah Detay</span>
          </button>
        </div>

        <!-- 3: Not -->
        <div class="detail-not">${item?.not || ""}</div>

        <!-- 4: Sınıf -->
        <div class="detail-sinif">${item?.sinif || ""}</div>

        <!-- 5: Sabah -->
        <div class="detail-sabah">${item?.sabah || ""}</div>

        <!-- 6: Akşam -->
        <div class="detail-aksam">${item?.aksam || ""}</div>
  ...
  function applyPagination() {
    ...
    buildSmartPages(totalPages, currentPage).forEach((p) => {
      ...
      $pagination.append(`
        <li class="page-item number ${p === currentPage ? "active" : ""}" data-page="${p}">
