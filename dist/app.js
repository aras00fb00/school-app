...
/* =========================================================
1) MENU / ACCORDION + MENU GUARDS — FIXED FINAL
========================================================= */
...
(function initMenuGuards() {
  // ✅ submenu link tıklanınca menüyü kapat (router click’i garanti için bir tick sonra)
  $("#accordion")
    .off("click.menuGuards", ".submenu a")
    .on("click.menuGuards", ".submenu a", function () {
      setTimeout(closeMenuUI, 0);
    });
  ...
  // link tıkında dış tık kapatma trigger olmasın
  $("#accordion")
...
/* =========================================================
2) PAGES (Dashboard ↔ Maps ↔ Students) — FIXED FINAL
========================================================= */
(function initPages() {
  ...
  function showMaps() {
    ...
    window.App.utils.raf2(() => {
      ...
      try {
        if (typeof window.setMapSearchVisible === "function") {
          window.setMapSearchVisible(isSchoolFixed());
        }
      } catch(e){}
    });
...
if (!window.__routerPagesBound) {
  ...
  // ✅ başlangıçta hangi sayfa aktifse ona göre UI (DÜZELTİLDİ: () eklendi)
...
function initPanelSearchModule(map) {
  ...
  // Sadece mousedown’ı engelle: click handler’ların çalışmasına izin ver
